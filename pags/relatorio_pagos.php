<?php
  ...
  $query = "SELECT * FROM despesas WHERE `date` BETWEEN '".$d_inicial."' AND '".$d_final."' AND `status` = '1' AND `pago` = '2' ORDER BY id asc";
  $busca = $PDO->prepare($query);
  $busca->execute();
  $fetch_busca = $busca->fetch(PDO::FETCH_ASSOC);

  //SQL PARA SOMAR VALORES SELECIONADOS
  $sqlsoma = "SELECT SUM(valor) AS total_global FROM despesas WHERE `date` BETWEEN '".$d_inicial."' AND '".$d_final."' AND `status` = '1' AND `pago` = '2'";
...
<head>
  <meta charset="utf-8">
  <title>GameTecZone - Sua melhor loja de game</title>
...
<body>
  <table class="relatorio_container">
    <thead>
      ...
      <tr>
        <th colspan="4"><h5>Relatório de contas pagas</h5></th>
      </tr>
      <tr>
        <th colspan="7"  class="texto_trelatorio">Relatório do período <?=date('d/m/Y', strtotime($d_inicial));?> a <?=date('d/m/Y', strtotime($d_final));?> de contas pagas retirado no dia <?=date('d/m/Y');?>.</th>
      </tr>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td colspan="2" class="valor"><?="R$ ".number_format($echo_total,2,',','.');?></td>
      </tr>
      <tr>
